...
public class FitnessWatches extends HttpServlet 
{
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
  {  
	SAXParserDataStore fetchProductList = new SAXParserDataStore();
	List<Products> fitnessWatchList = fetchProductList.getfitnessWatchList();
	...
	out.println("<li class='selected'><a href='fitnesswatches'>Fitness Watches</a></li>");	
	...
	out.println("<article>");
	out.println("<h2>FitnessWatches</h2>" +
				"<table>");
	
	Products p,p2;
	 for(int i = 0;i<fitnessWatchList.size();i++)
	{
		p = fitnessWatchList.get(i);
		...
		out.println("<tr>"+
					...
					"<input type='hidden' name='product' value='"+p+"'>" +
					"<input type='hidden' name='productCategory' value='FitnessWatches'>" +
					"</form>" +
